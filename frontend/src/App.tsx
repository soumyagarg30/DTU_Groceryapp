...
function App() {
  ...
  return <main>
    ...
    {data && <section className="results"><div className="results-head"><div><span className="section-label">{data.results.length} results</span><h2>{providersUnavailable ? 'Stores are unavailable' : data.results.length ? `Results for “${data.query}”` : `No results for “${data.query}”`}</h2><div className="freshness">{ageLabel(data.cache.age_seconds, data.cache.hit)}{data.cache.hit && ' · served from cache'}</div></div><div className="provider-status">{(['blinkit', 'zepto'] as ProviderName[]).map((provider) => <div key={provider} className={`provider-pill ${data.provider_status[provider]}`}><i /><span><strong>{providerLabel(provider)}</strong><small>{data.provider_messages?.[provider] === 'login_required' ? 'Sign-in required' : data.provider_status[provider] === 'ok' ? 'DTU verified' : data.provider_status[provider]}</small></span></div>)}</div></div>
